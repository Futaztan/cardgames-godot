...
public class Dealer
{
	...
	public Dealer(List<EntityBase> allPlayers)
	{
		...
		_cardPlaceSound = GD.Load<AudioStream>("res://Assets/Sound/card_deal.ogg");
	...
	public async Task DealCardsToPlayers(int missingCardNumFromPlayer)
	{
		
		Node mainScene = GetMainScene();
		Control cardDeck = mainScene.GetNode<Control>("Center/HBoxContainer1/CardDeck");
		int numOfDealedCards = Math.Min(missingCardNumFromPlayer, DeckSize / 4);
		for (int i = 0; i < numOfDealedCards; i++)
		{
